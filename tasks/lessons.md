...
### Lesson 57: テストが .pen SSOT と矛盾する場合はテストを修正する

**問題**: Legal ページのテストが「legal pages skip CTA」と断言していたが、.pen SSOT には ctaSection が存在していた。テストが誤っており、CTAセクションが実装から欠落していた。

**教訓**: テストは実装の正しさを保証するものだが、.pen SSOT が最終的な真実。テストと SSOT が矛盾する場合は常に SSOT を信頼し、テストを修正する。

### Lesson 58: .pen SSOT がないページは既存ページのCSS変数を再利用する
